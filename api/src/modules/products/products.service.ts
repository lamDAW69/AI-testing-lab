...
export class ProductsService {
  ...
  async listProducts(tenantId: string, limit: number, offset: number): Promise<Product[]> {
  ...
  async updateProduct(id: string, tenantId: string, input: UpdateProductInput): Promise<Product> {
    const updated = await this.repo.updateByIdAndTenant(id, tenantId, input);
  ...
  async deleteProduct(id: string, tenantId: string): Promise<void> {
    const deleted = await this.repo.deleteByIdAndTenant(id, tenantId);
